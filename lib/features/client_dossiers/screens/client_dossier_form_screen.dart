...
class ClientDossierFormScreen extends StatefulWidget {
  ...
  @override
  State<ClientDossierFormScreen> createState() => _ClientDossierFormScreenState();
}
...
class _ClientDossierFormScreenState extends State<ClientDossierFormScreen> {
  ...
  void dispose() {
    for (final ctrl in [
      _primaryNameCtrl, _familyNameCtrl, _emailCtrl, _phoneCtrl,
      _nationalityCtrl, _homeBaseCtrl, _groupDynamicCtrl, _beddingCtrl,
      _cuisineCtrl, _dislikesCtrl, _alcoholCtrl, _allergiesCtrl,
      _accessibilityCtrl, _securityCtrl, _photographyCtrl,
      _internalNotesCtrl, _feedbackCtrl, _serviceStyleCtrl, _flagsCtrl,
    ]) { ctrl.dispose(); }
    _scrollCtrl.dispose();
    super.dispose();
  }
  ...
  ClientDossier _buildDossier() {
    ...
    return ClientDossier(
      ...
      familyName:             _familyNameCtrl.text.trim().isNotEmpty ? _familyNameCtrl.text.trim() : null,
      email:                  _emailCtrl.text.trim().isNotEmpty ? _emailCtrl.text.trim() : null,
      phone:                  _phoneCtrl.text.trim().isNotEmpty ? _phoneCtrl.text.trim() : null,
      nationality:            _nationalityCtrl.text.trim().isNotEmpty ? _nationalityCtrl.text.trim() : null,
      homeBase:               _homeBaseCtrl.text.trim().isNotEmpty ? _homeBaseCtrl.text.trim() : null,
      typicalTripType:        _tripType,
      groupDynamicNotes:      _groupDynamicCtrl.text.trim().isNotEmpty ? _groupDynamicCtrl.text.trim() : null,
      pacingPreference:       _pacing,
      privacyPreference:      _privacy,
      luxuryLevel:            _luxury,
      guidePreference:        _guide,
      structurePreference:    _structure,
      accommodationType:      _accommodationType,
      beddingPreferences:     _beddingCtrl.text.trim().isNotEmpty ? _beddingCtrl.text.trim() : null,
      wellnessImportance:     _wellness,
      amenityPreferences:     _amenities.toList(),
      cuisinePreferences:     _split(_cuisineCtrl.text),
      diningDislikes:         _split(_dislikesCtrl.text),
      dietaryRestrictions:    _dietary.toList(),
      allergies:              _split(_allergiesCtrl.text),
      diningStyle:            _diningStyle,
      alcoholPreference:      _alcoholCtrl.text.trim().isNotEmpty ? _alcoholCtrl.text.trim() : null,
      ...
      heatTolerance:          _heat,
      walkingTolerance:       _walking,
      accessibilityNotes:     _accessibilityCtrl.text.trim().isNotEmpty ? _accessibilityCtrl.text.trim() : null,
      securitySensitivity:    _securityCtrl.text.trim().isNotEmpty ? _securityCtrl.text.trim() : null,
      photographySensitivity: _photographyCtrl.text.trim().isNotEmpty ? _photographyCtrl.text.trim() : null,
      internalNotes:          _internalNotesCtrl.text.trim().isNotEmpty ? _internalNotesCtrl.text.trim() : null,
      pastFeedbackNotes:      _feedbackCtrl.text.trim().isNotEmpty ? _feedbackCtrl.text.trim() : null,
      serviceStyleNotes:      _serviceStyleCtrl.text.trim().isNotEmpty ? _serviceStyleCtrl.text.trim() : null,
      operationalFlags:       _split(_flagsCtrl.text),
  ...
  static const _sectionTitles = [
    'Overview', 'Travel Style', 'Accommodation',
    'Dining', 'Experiences', 'Behavioral', 'Internal',
  ];
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: _isSaving ? AppColors.textMuted : AppColors.accent,
            ...
            child: SingleChildScrollView(
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    _FormSection(key: _sectionKeys[0], title: 'Overview',
                    ...
                    _FormSection(key: _sectionKeys[1], title: 'Travel Style',
                        child: _TravelStyleFields(
                          pacing:   _pacing,    onPacing:   (v) => setState(() => _pacing = v),
                          privacy:  _privacy,   onPrivacy:  (v) => setState(() => _privacy = v),
                          luxury:   _luxury,    onLuxury:   (v) => setState(() => _luxury = v),
                          guide:    _guide,     onGuide:    (v) => setState(() => _guide = v),
                          structure:_structure, onStructure:(v) => setState(() => _structure = v),
                        )),
                    const SizedBox(height: AppSpacing.xl),

                    _FormSection(key: _sectionKeys[2], title: 'Accommodation',
                    ...
                    _FormSection(key: _sectionKeys[3], title: 'Dining',
                        child: _DiningFields(
                          style:       _diningStyle,
                    ...
                    _FormSection(key: _sectionKeys[4], title: 'Experiences',
                        child: _ExperiencesFields(
                          cultural:     _cultural,     onCultural:     (v) => setState(() => _cultural = v),
                          adventure:    _adventure,    onAdventure:    (v) => setState(() => _adventure = v),
                          intellectual: _intellectual, onIntellectual: (v) => setState(() => _intellectual = v),
                          relaxation:   _relaxation,   onRelaxation:   (v) => setState(() => _relaxation = v),
                          shopping:     _shopping,     onShopping:     (v) => setState(() => _shopping = v),
                        )),
                    const SizedBox(height: AppSpacing.xl),

                    _FormSection(key: _sectionKeys[5], title: 'Behavioral & Comfort',
                        child: _BehavioralFields(
                          prefersLate:       _prefersLateStarts,
                          onPrefersLate:     (v) => setState(() => _prefersLateStarts = v),
                          dislikesCrowds:    _dislikesCrowds,
                          onDislikesCrowds:  (v) => setState(() => _dislikesCrowds = v),
                          heat:    _heat,    onHeat:    (v) => setState(() => _heat = v),
                          walking: _walking, onWalking: (v) => setState(() => _walking = v),
                    ...
                    _FormSection(key: _sectionKeys[6], title: 'Internal Notes',
                        isInternal: true,
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            if (isInternal) ...[
              Icon(Icons.lock_outline_rounded, size: 11, color: AppColors.accent),
...
class _OverviewFields extends StatelessWidget {
  final TextEditingController primaryCtrl, familyCtrl, emailCtrl, phoneCtrl,
      nationalityCtrl, homeBaseCtrl, groupCtrl;
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        _gap,
        _enumRow('Trip type', TripType.values, tripType, (t) => t.label, onTripType),
        _gap,
...
class _TravelStyleFields extends StatelessWidget {
  final PacingPreference?    pacing;    final ValueChanged<PacingPreference?> onPacing;
  final PrivacyPreference?   privacy;   final ValueChanged<PrivacyPreference?> onPrivacy;
  final LuxuryLevel?         luxury;    final ValueChanged<LuxuryLevel?> onLuxury;
  final GuidePreference?     guide;     final ValueChanged<GuidePreference?> onGuide;
  final StructurePreference? structure; final ValueChanged<StructurePreference?> onStructure;

  const _TravelStyleFields({
    required this.pacing,   required this.onPacing,
    required this.privacy,  required this.onPrivacy,
    required this.luxury,   required this.onLuxury,
    required this.guide,    required this.onGuide,
    required this.structure,required this.onStructure,
  });
  ...
  Widget build(BuildContext context) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      _enumRow('Pacing', PacingPreference.values, pacing, (v) => v.label, onPacing),
      _gap,
      _enumRow('Privacy', PrivacyPreference.values, privacy, (v) => v.label, onPrivacy),
      _gap,
      _enumRow('Luxury level', LuxuryLevel.values, luxury, (v) => v.label, onLuxury),
      _gap,
      _enumRow('Guide preference', GuidePreference.values, guide, (v) => v.label, onGuide),
      _gap,
      _enumRow('Structure', StructurePreference.values, structure, (v) => v.label, onStructure),
    ],
  );
}
...
class _AccommodationFields extends StatelessWidget {
  final AccommodationType? type;  final ValueChanged<AccommodationType?> onType;
  final WellnessImportance? wellness; final ValueChanged<WellnessImportance?> onWellness;
  final TextEditingController beddingCtrl;
  final Set<String> amenities; final ValueChanged<Set<String>> onAmenities;

  const _AccommodationFields({
    required this.type, required this.onType,
    required this.wellness, required this.onWellness,
    required this.beddingCtrl,
    required this.amenities, required this.onAmenities,
  });

  static const _amenityOptions = [
    'Private pool', 'Beach access', 'Gym', 'Spa', 'Butler service',
    'Kids club', 'Restaurant on site', 'Quiet property',
  ];
  ...
  Widget build(BuildContext context) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      _enumRow('Property type', AccommodationType.values, type, (v) => v.label, onType),
      _gap,
      _enumRow('Wellness / spa', WellnessImportance.values, wellness, (v) => v.label, onWellness),
      _gap,
      _field('Bedding preferences', beddingCtrl),
      _gap,
      _multiChipRow('Amenity priorities', _amenityOptions, amenities, onAmenities),
    ],
  );
}
...
class _DiningFields extends StatelessWidget {
  final DiningStyle? style; final ValueChanged<DiningStyle?> onStyle;
  final TextEditingController cuisineCtrl, dislikesCtrl, alcoholCtrl, allergiesCtrl;
  final Set<String> dietary; final ValueChanged<Set<String>> onDietary;

  const _DiningFields({
    required this.style, required this.onStyle,
    required this.cuisineCtrl, required this.dislikesCtrl,
    required this.alcoholCtrl, required this.allergiesCtrl,
    required this.dietary, required this.onDietary,
  });

  static const _dietaryOptions = [
    'Vegetarian', 'Vegan', 'Gluten-free', 'Dairy-free',
    'Halal', 'Kosher', 'No pork', 'No shellfish',
  ];
  ...
  Widget build(BuildContext context) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      _enumRow('Dining style', DiningStyle.values, style, (v) => v.label, onStyle),
      _gap,
      _field('Cuisine preferences (comma-separated)', cuisineCtrl),
      _gap,
      _field('Dining dislikes (comma-separated)', dislikesCtrl),
      _gap,
      _multiChipRow('Dietary restrictions', _dietaryOptions, dietary, onDietary),
      _gap,
...
class _ExperiencesFields extends StatelessWidget {
  ...
  final ValueChanged<int> onCultural, onAdventure, onIntellectual,
      onRelaxation, onShopping;

  const _ExperiencesFields({
    required this.cultural,     required this.onCultural,
    required this.adventure,    required this.onAdventure,
    required this.intellectual, required this.onIntellectual,
    required this.relaxation,   required this.onRelaxation,
    required this.shopping,     required this.onShopping,
  });
...
class _BehavioralFields extends StatelessWidget {
  final bool prefersLate;  final ValueChanged<bool> onPrefersLate;
  final bool dislikesCrowds; final ValueChanged<bool> onDislikesCrowds;
  final HeatTolerance? heat;    final ValueChanged<HeatTolerance?> onHeat;
  final WalkingTolerance? walking; final ValueChanged<WalkingTolerance?> onWalking;
  final TextEditingController accessCtrl, securityCtrl, photoCtrl;

  const _BehavioralFields({
    required this.prefersLate, required this.onPrefersLate,
    required this.dislikesCrowds, required this.onDislikesCrowds,
    required this.heat, required this.onHeat,
    required this.walking, required this.onWalking,
    required this.accessCtrl, required this.securityCtrl,
  ...
  Widget build(BuildContext context) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      _toggle('Prefers late morning starts (10am+)', prefersLate, onPrefersLate),
      _toggle('Sensitive to crowds', dislikesCrowds, onDislikesCrowds),
      _gap,
      _enumRow('Heat tolerance', HeatTolerance.values, heat, (v) => v.label, onHeat),
      _gap,
      _enumRow('Walking tolerance', WalkingTolerance.values, walking, (v) => v.label, onWalking),
      _gap,
